...
{
	sealed internal partial class WordsComplexAnalyzer
	{
		[TestFixture]
		internal class WordsComplexAnalyzer_Tests
		{
			[Test]
			...
			{
				var analyzer = new WordsComplexAnalyzer();
				foreach(char c in text)
					analyzer.TreatChar(c);
				analyzer.Finish();
				Assert.AreEqual(wordsExpected, analyzer.wordsCount);
			}
		}
	}
}